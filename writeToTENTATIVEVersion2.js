...
updatedUpdatedUpdatedUdatedUpdatedUpdatedUpdatedActiveStudentDataMap,
  ...
  updatedUpdatedUpdatedUdatedUpdatedUpdatedUpdatedActiveStudentDataMap.forEach(
    (studentData, studentId) => {
      ...
      let tentativeArray = studentData["TENTATIVE"];
      if (!tentativeArray || tentativeArray.length === 0 || !tentativeArray[0]) {
        console.warn(`Student ID ${studentId}: Missing or invalid tentativeArray data. Using alternate data to build this student's row.`);
      ...
      if (
        ...
        entryWithdrawalArray = [{}]; // Create an empty object for this student if Entry_Withdrawal data is not available
      }
      ...
      const fullName = studentData["Entry_Withdrawal"][0]["Student Name(Last, First)"];
      ...
      function updateTeacherInput(
        ...
        const flatSchedulesArray = Array.isArray(schedulesArray) ? flattenArray(schedulesArray) : [];

          if (flatSchedulesArray.length === 0) {
            console.warn(`${studentId}: schedulesArray is empty or invalid.`);
          }
          ...
          if (teacherInput.hasOwnProperty(period)) {
            ...
            if (matchingEntry) {
              ...
              teacherInput[period]["Course Title"] =
                ...
                (tentativeArray?.[0] ? tentativeArray[0][`${period} Period - Course Title`] : "Line 274, empty string") ||
                "Line 274, empty string";
            } else {
      ...
      if (!tentativeArray || tentativeArray.length === 0) tentativeArray = [{}];
      if (!entryWithdrawalArray || entryWithdrawalArray.length === 0) entryWithdrawalArray = [{}];

      // Store the first element of each array safely
      const tentativeEntry = tentativeArray?.[0] || {}; // Safe to use
      Logger.log(typeof(tentativeEntry))
      const entryWithdrawalEntry = entryWithdrawalArray[0]; // Safe to use

      outputData.push([
        tentativeEntry?.["DATE ADDED TO SPREADSHEET"]
          ? formatDateToMMDDYYYY(tentativeEntry["DATE ADDED TO SPREADSHEET"])
          : formatDateToMMDDYYYY(new Date()),
        lastName,
        firstName,
        studentId || null,
        tentativeEntry?.["GRADE"] || entryWithdrawalEntry?.["Grd Lvl"],

        teacherInput["1st"]["Course Title"], // "1st Period - Course Title"
        teacherInput["1st"]["Teacher Name"], // "1st Period - Teacher Name"
        tentativeEntry?.["1st Period - Transfer Grade"] ?? "", // "1st Period - Transfer Grade"
        tentativeEntry?.["1st Period - Current Grade"] ?? "", // "1st Period - Current Grade"
        teacherInput["1st"]["How would you assess this student's academic growth?"],
        ...
        tentativeEntry?.["2nd Period - Transfer Grade"] ?? "", // "2nd Period - Transfer Grade"
        tentativeEntry?.["2nd Period - Current Grade"] ?? "", // "2nd Period - Current Grade"
        teacherInput["2nd"]["How would you assess this student's academic growth?"],
        ...
        tentativeEntry?.["3rd Period - Transfer Grade"] ?? "", // "3rd Period - Transfer Grade",
        tentativeEntry?.["3rd Period - Current Grade"] ?? "", // "3rd Period - Current Grade",
        teacherInput["3rd"]["How would you assess this student's academic growth?"],
        ...
        tentativeEntry?.["4th Period - Transfer Grade"] ?? "", // "4th Period - Transfer Grade",
        tentativeEntry?.["4th Period - Current Grade"] ?? "", // "4th Period - Current Grade",
        teacherInput["4th"]["How would you assess this student's academic growth?"],
        ...
        tentativeEntry?.["5th Period - Transfer Grade"] ?? "", // "5th Period - Transfer Grade",
        tentativeEntry?.["5th Period - Current Grade"] ?? "", // "5th Period - Current Grade",
        teacherInput["5th"]["How would you assess this student's academic growth?"],
        ...
        tentativeEntry?.["6th Period - Transfer Grade"] ?? "", // "6th Period - Transfer Grade",
        tentativeEntry?.["6th Period - Current Grade"] ?? "", // "6th Period - Current Grade",
        teacherInput["6th"]["How would you assess this student's academic growth?"],
        ...
        tentativeEntry?.["7th Period - Transfer Grade"] ?? "", // "7th Period - Transfer Grade",
        tentativeEntry?.["7th Period - Current Grade"] ?? "", // "7th Period - Current Grade",
        teacherInput["7th"]["How would you assess this student's academic growth?"],
        ...
        tentativeEntry?.["8th Period - Transfer Grade"] ?? "", // "8th Period - Transfer Grade",
        tentativeEntry?.["8th Period - Current Grade"] ?? "", // "8th Period - Current Grade",
        teacherInput["8th"]["How would you assess this student's academic growth?"],
        ...
        "", // "SE - What are the student's needs, as far as behavior?"
        "", // "SE - What are the student's needs, as far as functional skills?"
        "", // "SE - Please add any other comments or concerns here:"

        studentData.Registrations_SY_24_25?.[0]?.["Home Campus"] || null, // "REGULAR CAMPUS"
        formattedEntryDate, // "FIRST DAY OF AEP"
        formatDateToMMDDYYYY(estimatedExitDay) || null, // "Anticipated Release Date"
        "", // "Parent Notice Date"
        "", // "Withdrawn Date"
        "", // "Attendance Recovery"
        studentData.Registrations_SY_24_25?.[0]?.["Eligibilty"] || null, // "COMPASS"
        "", // "Credit Retrieval"
        studentData.Registrations_SY_24_25?.[0]?.["Behavior Contract"] || null, // "Behavior Contract"
        "", // "Campus Mentor"
        ...
        studentData.ContactInfo?.[0]?.["Student Email"] || null, // "Student Email"
        studentData.ContactInfo?.[0]?.["Parent Name"] || null, // "Guardian Name"
        studentData.ContactInfo?.[0]?.["Guardian 1 Email"] || null, // "Guardian Email"
