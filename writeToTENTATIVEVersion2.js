  ...
  updatedUpdatedUpdatedUdatedUpdatedUpdatedUpdatedActiveStudentDataMap.forEach(
    (studentData, studentId) => {
      ...
      if (!tentativeArray || tentativeArray.length === 0 || !tentativeArray[0]) {
        console.warn(`Student ID ${studentId}: Missing or invalid tentativeArray data. Skipping this student.`);
        return; // Skip processing for this student
      }
      ...
      if (tentativeArray && entryWithdrawalArray) {
        outputData.push([
          tentativeArray &&
          tentativeArray[0] &&
        ...
        console.warn(`Student ID ${studentId}: Missing required data from output. Skipping.`);
      }
    },
  );
